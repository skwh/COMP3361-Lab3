...
FrameAllocator::FrameAllocator(unsigned long page_count) {
    ...
    store_in_memory32(page_count, PAGE_FRAMES_TOTAL_OFFSET);
    //store the avaliable page frames in memory: the 0th frame is reserved
    store_in_memory32(page_count - 1, PAGE_FRAMES_AVALIABLE_OFFSET);
    //store the address of the next page frame in memory, always the page frame size
    store_in_memory32(PAGE_FRAME_SIZE, AVALIABLE_LIST_HEAD_OFFSET);
    ...
    while (current_address < page_count * PAGE_FRAME_SIZE) {
       ...
       if (next_address >= page_count * PAGE_FRAME_SIZE) {
           store_in_memory32(0xFFFFFFFF, current_address);
           break;
       }
       store_in_memory32(next_address, current_address);
...
bool FrameAllocator::allocate(uint32_t count, std::vector<uint32_t> &page_frames) {
    ...
    while (address_count > 0) {
        ...
        old_address = address;
        load_from_memory32(address, address);
        set_frame_pointer(0, old_address);
        address_count--;
    }
    set_frames_avaliable(get_avaliable() - count);
    store_in_memory32(address, AVALIABLE_LIST_HEAD_OFFSET);
    return true;
}
...
bool FrameAllocator::release(uint32_t count, std::vector<uint32_t> &page_frames) {
    ...
    while (page_frames.size() != 0) {
        ...
        store_in_memory32(end_address, AVALIABLE_LIST_HEAD_OFFSET);
...
uint32_t FrameAllocator::get_avaliable() const {
    uint32_t count = 0;
    load_from_memory32(count, PAGE_FRAMES_AVALIABLE_OFFSET);
    return count;
}
...
std::string FrameAllocator::get_avaliable_list_string() const {
    ...
    load_from_memory32(read_address, AVALIABLE_LIST_HEAD_OFFSET);
    while (read_address != 0xFFFFFFFF) {
        list_string << std::hex << read_address << " ";
        load_from_memory32(read_address, read_address);
    }
    return list_string.str();
}

void FrameAllocator::store_in_memory8(uint8_t value, uint32_t address) {
    this->memory[address] = value;
}

void FrameAllocator::load_from_memory8(uint8_t &value, uint32_t address) const {
    value = this->memory[address];
}

void FrameAllocator::store_in_memory32(uint32_t value, uint32_t address) {
    memcpy(&(this->memory[address]), &value, sizeof(uint32_t));
}

void FrameAllocator::load_from_memory32(uint32_t &value, uint32_t address) const {
    memcpy(&value, &(this->memory[address]), sizeof(uint32_t));
}

void FrameAllocator::set_frames_avaliable(uint32_t value) {
    store_in_memory32(value, PAGE_FRAMES_AVALIABLE_OFFSET);
}

void FrameAllocator::set_frame_pointer(uint32_t value, uint32_t address) {
    store_in_memory32(value, address);
}

void FrameAllocator::get_head(uint32_t &value) {
    load_from_memory32(value, AVALIABLE_LIST_HEAD_OFFSET);
}
